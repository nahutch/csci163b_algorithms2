...
public:
   ...
   std::vector<std::string> find_euler_path() const
   {
       std::vector<T> ans;
       std::stack<T> S;
       std::unordered_map<T, AdjacentType> A;
       ...
       // find odd numbered edges in in the graph, make it the start, quit if < 2 verticies with odd degree
       T v = "";
       ...
       //check if more than 2 odd degree edges or a 0 degree edge
       if (check > 2 || is_connected())
       {
           ans.push_back("No valid Euler Path");
           return ans;
       }
       //if no odd degree edges, define start to be first element
       else if (check==0)
       {
           v = _t.begin()->first;
       }
       ...
       while (!done)
       {
           while (true)
           {
              ...
              std::unordered_set<T>::iterator i =  A[v].begin();

               //go to verticies with an even number of edges first, only go to an odd number if forced to do so.
       ...
       //tried another method of solving this assignment, but it did not work so I started over using
       //euler cycle to start me off, but I ran out of time
       ...
       T start= "";
       std::unordered_map<T, AdjacentType> map1;
       std::size_t check(0);
       ...
       if (check > 2 || is_connected())                                   //check if more than 2 odd degree edges or a 0 degree edge
       {
           ans.push_back("No valid Euler Path");
           return ans;
       }
       else if (check==0)                                //if no odd degree edges, define start to be anywhere
       {
           for (auto e: _t)
           {
               start=e.first;
       ...
       std::unordered_map<T, AdjacentType>::iterator i = map1.begin();

       while (map1.empty() == false)                     //run until all edges have been taken
       {

           while (i!=map1.end())
           {
               //std::cout<< i->first << std::endl;
               map1.at(i->first).clear();
               i++;
           }
           ...
           while (map1.empty() == false)                                //use i->first or i->second
           {
               std::cout<< "is vertex "<< i->first << " empty? "<< i->first.empty()<< std::endl;
               if (i->second.empty() == true)
               {
                   std::cout <<"yes, so deleting this node: " << i->first << std::endl;
                   map1.erase(i->first);
               }
               ...
               i++;
                                                  ...
                                                  //if no edges on a vertex, remove the vertex
