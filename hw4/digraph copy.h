...
public:
    ...
    {
        std::size_t a = n();
        std::vector<std::vector<std::size_t>> matrix(a, std::vector<std::size_t>(a,a));
        std::unordered_map< T, std::size_t> map1;
        ...
        //put 1 in the matrix where there is an edge
        for (auto e: map1)
        {

            for (auto f: Adj(e.first))
            {
                matrix[e.second][map1[f]]=1;
            }

        }


        //Augmented Floyd's algorithm
        for (std::size_t i = 0; i < n(); i++)
        {
            for (std::size_t j = 0; j < n(); j++)
            {
