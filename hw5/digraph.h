...
public:
   ...
   */
    ...
    std::unordered_map<std::string, std::size_t> Tarjan_scc() const
    {
        std::unordered_map<std::string, std::size_t> pre, post, low, cc;
        std::size_t time(1), count(1);
        std::stack<T> st;
        T temp= "";
        ...
        {
            if (pre.count(s) == 0)
                Tarjan_dfs(s, pre, post, low, cc, st, time, count);
        }
    ...
    void Tarjan_dfs(const T & v,
              std::unordered_map<std::string, std::size_t> & pre,
              std::unordered_map<std::string, std::size_t> & post,
              std::unordered_map<std::string, std::size_t> & low,
              std::unordered_map<std::string, std::size_t> & cc,
              std::stack<T> & st,
              std::size_t & time,
              std::size_t & count) const
    {
        ...
        for (auto w: Adj(v))
        {
            if (pre.find(w) == pre.end())
            {
                Tarjan_dfs(w, pre, post, low, cc, st, time, count);
                low[v] = std::min(low[v], low[w]);
            }
            else if(post.count(w)==0)
            {
                    //checking if child is a backedge
        ...
        //checking if in a connected component
        if (pre[v]==low[v])
        {
            while (st.top()!=v)
            {
